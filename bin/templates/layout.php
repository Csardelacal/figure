...
<html>
	<head>
		<title><?= isset(${'page.title'}) && ${'page.title'}? ${'page.title'} : 'Figure - Image uploads' ?></title>
	...
	<body>
		...
		(function () {
			depend(['m3/depend/router'], function(router) {
				...
				router.equals('phpas/app/drawer').to( function() { return '<?= $authSSO->getAppDrawerJS() ?>'; });
		...
		<script type="text/javascript">
			...
			depend(['m3/core/request'], function (Request) {
				// I don't like the way I fixed this. But for now it's patched up.
				var request = new Request('<?= $authSSO->getAppDrawerJS() ?>on');
				request
					.then(JSON.parse)
					.then(function (e) {
